...
class AppointmentController extends Controller
{
    ...
    public function store_edit(Request $request){
       ...
       /// print_r($request->update_time); die(); exit;

        $appointment = Appointment::findOrFail($request->rdv_id);
       if($request->rdv_status){ $appointment->visited = $request->rdv_status; }
        if($request->update_time){$appointment->time_start = $request->update_time;}
        if($request->update_date){$appointment->date = $request->update_date;}
        $appointment->save();
